...
//      echo '<pre>'.print_r($config, true).'wtf1</pre>';
    ...
    */
   public function ExecuteQuery($query, $fetchMode = MYSQLI_ASSOC){
...
//      $this->CreateLogEntry($this->query, 'debug');
      ...
      $result = $this->ExecuteQuery($this->query, MYSQL_ASSOC);
      if($result == 1){
         $this->CreateLogEntry("There was an error while fetching data from the database.", 'fatal', true);
         $this->lastError = "There was an error while fetching data from the session database.<br />Please try again later.a";
         return 1;
      }
      ...
      if(count($result) == 0){
         ...
         if($res == -2){
            $this->CreateLogEntry("There was an error while fetching data from the database.", 'fatal', true);
            $this->lastError = "There was an error while fetching data from the session database.<br />Please try again later.b";
            return 1;
         }
         elseif(is_null($res)) return 3;   //we do not have an account with that username
         else{
            //check if the account is disabled
            $res = $this->GetSingleRowValue(Config::$config['session_dbase'] . '.users', 'allowed', 'login', $username);
            if($res == -2){
               $this->CreateLogEntry("There was an error while fetching data from the database.", 'fatal', true);
               $this->lastError = "There was an error while fetching data from the session database.<br />Please try again later.c";
               return 1;
            }
...
//      $this->Dbase->CreateLogEntry(">session.read:\n ".print_r($_SESSION, true), 'debug');
      ...
      $query = 'SELECT updated_at, data FROM ' . Config::$config['session_dbase'] . ".sessions WHERE session_id='$sessionId'";
