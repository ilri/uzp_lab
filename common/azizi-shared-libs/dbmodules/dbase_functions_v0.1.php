...
global $dbcon, $config, $query;
   ...
   //echo "gj";
	$query = "REPLACE INTO ".$config['session_dbase'].".sessions(session_id, data, updated_at) VALUES ('".mysql_real_escape_string($sid)."', '".mysql_real_escape_string($data)."','$time')";
	$result = mysql_query($query, $dbcon);
   //LogError($query);
...
global $dbcon, $query, $paging, $psswdSettings, $config;
   ...
   if($psswdSettings['useSalt']){
      $query="SELECT * FROM ".$config['session_dbase'].".users WHERE login='$username' AND psswd=sha1(concat('$username',salt,'$password')) AND allowed='1' LOCK IN SHARE MODE";
   }
   else{
      $query="SELECT * FROM users WHERE login='$username' AND psswd='$password' AND allowed='1' LOCK IN SHARE MODE";
   }
   //echo $query.'<br>';
   $result=mysql_query($query, $dbcon);
   if(!$result){
...
//   	if(!mkdir("$dir/",0777)){
//   		$err_occ=1;
//   		die("There was an error while creating the folder ".getcwd()."$dir for saving the error logs.$contact");
//   	}
//   }
   ...
   if(eregi('Debugging',$error)!==false) fputs($fd, "{$errorString}['Debug'] $query\n");
