...
//      echo '<pre>'.print_r($this, true).'</pre>'; die();
    ...
    */
   public function DeleteData($table, $col, $colval){
      if(is_array($col)){
         $con='';
...
//      echo '<pre>'.print_r($this, true).'</pre>';
      ...
      $query = "select a.sname, a.onames, c.value as project, b.name as user_level from {$this->config['session_dbase']}.users as a
               inner join {$this->config['session_dbase']}.user_levels as b on a.user_level=b.id
               inner join azizi.modules_custom_values as c on a.project=c.val_id ";
      ...
      $_SESSION['username'] = $username; $_SESSION['project'] = $userData['project'];
      $_SESSION['surname'] = $userData['sname']; $_SESSION['onames'] = $userData['onames']; $_SESSION['user_level'] = $userData['user_level'];
      return 0;
   }
...
//      echo $this->query;
      $query = "select a.id, c.value as project, b.name as user_level from {$this->config['session_dbase']}.users as a
               inner join {$this->config['session_dbase']}.user_levels as b on a.user_level=b.id
               inner join azizi.modules_custom_values as c on a.project=c.val_id where login='$username' and ldap_authentication=1 and allowed=1";
      $res = $this->ExecuteQuery(MYSQLI_ASSOC);
...
//      var_dump($data);
//      echo "<pre>session.write: ".print_r($data, true).'</pre>';
