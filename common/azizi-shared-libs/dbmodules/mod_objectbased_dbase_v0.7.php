...
//      echo '<pre>'.print_r($config, true).'wtf1</pre>';
      if(!isset (Config::$config)){   //assume we have some inherited config
...
//      echo "<pre>".print_r(Config::$logLevels[$level], true)."</pre>";
      ...
      if(Config::$logLevels[$level][0] == '') fputs($fd, "$err\n");
      else fputs($fd, $messageString . Config::$logLevels[$level][0] . " $err\n\n");
      fclose($fd);
      return  0;
   }
...
//      $this->CreateLogEntry('Last query:', 'debug', true);
...
//      $this->Dbase->CreateLogEntry(">session.read.data:\n ".print_r($result, true), 'debug');
      ...
      if(count($result) == 0){   //we dont have an initialized session
         ...
         if(floor($secsDiff/60) > Config::$timeout){
            ...
            $this->CreateLogEntry('session time expired', 'debug');
