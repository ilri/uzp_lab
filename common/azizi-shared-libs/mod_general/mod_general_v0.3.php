 ...
 * - Added a function to create the snapshot of the db
 */
...
//         echo "Moving: $curFile => $destination<br />";
    ...
    * @param   string   $dbase         A string with the database name that we are taking the snapshot
    ...
    * @since   v0.3
    */
   public function CreateDbSnapshot($config, $dbase, $backupFolder, $description = '') {
//      echo "backup folder -- $backupFolder <br />";
      if($backupFolder == '') return 'Please specify a valid backup folder.';
      if($dbase == '') return 'Please specify a database name to backup.';
      ...
      if(strlen($description) > 20) $description = substr($description, 0, 29);
